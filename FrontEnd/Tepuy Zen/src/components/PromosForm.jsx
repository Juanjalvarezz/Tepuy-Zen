...
const PromosForm = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:3000/api/promos', formData);
    ...
    <div className="w-4/5 mx-auto flex justify-center items-center">
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto mt-5 p-5 rounded-lg bg-[#77BFA3]">
        ...
        <input
          ...
          name="duracion"
          value={formData.duracion || ''} // Usar un string vacío si formData.duracion es null o undefined
          onChange={handleChange}
          max={10}
          className="w-full p-2 border mb-2 rounded-lg"
          required // Campo requerido
        ...
        <div className="mb-3">
        <h1 className='md:col-span-2 text-center -mt-2 mb-1'>Agrega una Imagen para mostrar la Promo:</h1>
